...
(defun test-send1 ()
  ...
  (with-connection ()
    ...
    (amqp-publish "Hello from CL-BUNNY"
             :routing-key "xx")))
...
(defun test-recv1 ()
  (with-connection (:channel-id 1)
    ...
    (cl-rabbit:basic-consume *connection* *channel* "xx")
    (let* ((result (cl-rabbit:consume-message *connection*))
           (message (cl-rabbit:envelope/message result)))
      (format t "Got message: ~s~%content: ~s~%props: ~s"
              result (babel:octets-to-string (cl-rabbit:message/body message) :encoding :utf-8)
              (cl-rabbit:message/properties message))
      (cl-rabbit:basic-ack *connection* *channel* (cl-rabbit:envelope/delivery-tag result)))))
